...
'''
2023.1.8：修改替换操作，将out.div_(torch.sum(weights))修改为out = torch.div(out, torch.sum(weights))防止计算图消失
'''
...
def weighted_mean(tensor, dim=None, weights=None):
    ...
    if dim is None:
        ...
        out = torch.div(out, torch.sum(weights))
    else:

        mean_dim = torch.sum(tensor * weights, dim=dim)

        mean_dim = torch.div(mean_dim, torch.sum(weights, dim=dim))
