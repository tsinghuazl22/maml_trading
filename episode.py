...
class BatchEpisodes2(object):
    ...
    def observations(self):
        if self._observations is None:
            observation_shape = self._observations_list[0][0][0].shape
            ...
            for i in range(self.batch_size):
                ...
                observations[:length, i] = np.stack(self._observations_list[i], axis=0).reshape(length,-1)
    ...
    def actions(self):
        ...
        return self._actions.reshape(len(self),self.batch_size)
    ...
    def append(self, observations, actions, rewards, batch_ids):
        self._observations_list[batch_ids].append(observations.astype(np.float32))
        self._actions_list[batch_ids].append(actions.astype(np.float32))
        self._rewards_list[batch_ids].append(rewards.astype(np.float32))
